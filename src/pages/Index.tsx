...
const UserRow: React.FC<{ user: UserCounts; position: number }> = React.memo(
  ({ user, position }) => {
    ...
    return (
      <tr className="border-b border-gray-800 hover:bg-gradient-to-r hover:from-gray-900 hover:to-gray-800 transition-colors duration-300 transform hover:scale-[1.02] cursor-pointer">
        ...
        <td className="py-4 px-6 text-center font-semibold text-white">{user.totalCount}</td>
      </tr>
    );
  }
);
...
const Index: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const sorted = Array.from(countsMap.values()).sort(
  ...
  const topUser = useMemo(() => {
    return leaderboard.length > 0 ? leaderboard[0].username : 'dmztdhruv';
  }, [leaderboard]);
              ...
              <table className="w-full table-fixed border-collapse">
                <thead>
                  <tr className="border-b border-gray-700">
                    <th className="text-left py-4 px-6 text-gray-500 font-semibold tracking-wide">
                      Position
                    </th>
                    <th className="text-left py-4 px-6 text-gray-500 font-semibold tracking-wide">
                      User
                    </th>
                    <th className="text-center py-4 px-6 text-gray-500 font-semibold tracking-wide">
                      PR Count
                    </th>
                    <th className="text-center py-4 px-6 text-gray-500 font-semibold tracking-wide">
                      Issue Count
                ...
                <tbody>
                  {leaderboard.map((user, idx) => (
                    <UserRow key={user.username} user={user} position={idx + 1} />
                  ))}
                </tbody>
              </table>
            )}
